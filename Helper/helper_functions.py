        ...
        try:
            await bot.send_message(
                ...
                buttons=[
                    Button.inline(
                        "Download",
                        data=f"Download:{id}:{search_details.get('episodes')}",
                    )
                ],
            )
        ...
        try:
            await bot.send_message(
                ...
                buttons=[
                    Button.inline(
                        "Download",
                        data=f"longdl:{split_id[1]}:{id[-25:]}:{search_details.get('episodes')}",
                    )
                ],
            )
        except BaseException:
...
async def send_download_link(event, id, ep_num):
    ...
    if "status" in result:
        ...
        await bot.send_message(
            event.chat_id, f"Download Links for episode {ep_num}\n{result}"
        )
